...
#Sensor range selection
  ...
  def set_range(self,range_r):
    ...
    if range_r == self.RANGE_6G:
     reg = reg&(~(3<<4));
    elif range_r == self.RANGE_12G:
     reg = reg&(~(3<<4));
     reg = reg | (0x01<<4);
    elif range_r == self.RANGE_24G:
     reg = reg&(~(3<<4));
     reg = reg | (0x03<<4);
    #print(reg)
...
'''
class DFRobot_LIS331HH_I2C(DFRobot_LIS331HH): 
  def __init__(self ,bus ,addr):
    self.__addr = addr
    super(DFRobot_LIS331HH_I2C, self).__init__(bus,0)
